...
function renderMovieCard(movie) {
    ...
    movieCard.innerHTML = `
        <img src="${imageUrl}" alt="${movie.title || "Pelicula sin titulo"}" class="w-full h-64 object-cover">
        <div class="p-4">
...
async function fetchAndRenderMovies(searchTerm = "", genreId = "", page = 1) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error(`Error HTTP! Estado: ${response.status}, Mensaje: ${errorText}`);
        ...
        if (movies.length === 0) {
            showMessage("No se encontraron películas con los criterios seleccionados.", "info");
